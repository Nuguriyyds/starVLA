...
原 48 请求在 worker=0/2 均全部通过，图像、PTS、样本身份一致；每轮 192 路中 81 路使用余量，与此前捕获候选中的最近合格 PTS 全部一致。时间策略 12 项单测通过，包括旧策略、余量上限、等距、距离拒绝与解码错误传播。另用私人临时无效 MP4 调用生产 `_video_frame()`，实际抛出 `InvalidDataError`，未接受损坏文件。
